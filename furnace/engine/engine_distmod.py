...
class Engine(object):
    def __init__(self, custom_parser=None):
        ...
        if self.distributed:
            ...
            dist.init_process_group(backend="nccl", init_method='env://',world_size = self.world_size,rank = int(os.environ['RANK']))
    ...
    def inject_default_parser(self):
        ...
        p.add_argument('-c', '--continue', type=str,
                       dest="continue_fpath",
                       # default = '/home/chenrui/code/TorchSemiSeg2/AAA_test/epoch-51.pth',
                       help='continue from one certain checkpoint')
    ...
    def save_checkpoint(self, path):
        ...
        if self.state.optimizer_D1 is not None:
            state_dict['optimizer_D1'] = self.state.optimizer_D1.state_dict()
        if self.state.optimizer_D2 is not None:
            state_dict['optimizer_D2'] = self.state.optimizer_D2.state_dict()
        if self.state.model_D1 is not None:
            state_dict['model_D1'] = self.state.model_D1.state_dict()
        if self.state.model_D2 is not None:
            state_dict['model_D2'] = self.state.model_D2.state_dict()
    ...
    def restore_checkpoint(self):
        ...
        self.state.model = load_model(self.state.model, tmp['model'],
                                      True)
        if 'model_D1' in tmp:
            self.state.model_D1 = load_model(self.state.model_D1, tmp['model_D1'],
                                      True)
        if 'model_D2' in tmp:
            self.state.model_D2 = load_model(self.state.model_D2, tmp['model_D2'],
                                      True)
        ...
        if 'optimizer_D1' in tmp:
            self.state.optimizer_D1.load_state_dict(tmp['optimizer_D1'])
        if 'optimizer_D2' in tmp:
            self.state.optimizer_D2.load_state_dict(tmp['optimizer_D2'])
        if 'optimizer' in tmp:
