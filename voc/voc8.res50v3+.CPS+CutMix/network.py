...
import os
import sys
sys.path.append('/home/chenrui/code/TorchSemiSeg2/furnace/')
import cv2
import torch
...
class SingleNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        ...
        # self.head2 = Head(num_classes, norm_layer, config.bn_momentum)
        self.business_layer = []
        self.business_layer.append(self.head)
        self.criterion = criterion

        self.classifier = nn.Conv2d(256, num_classes, kernel_size=1, bias=True)
        # self.classifier2 = nn.Conv2d(256, 24, kernel_size=4, stride = 2,bias=True)

        self.business_layer.append(self.classifier)

    def forward(self, data):
        blocks = self.backbone(data)
        aspp_feat,v3plus_feature = self.head(blocks)      # (b, c, h, w)
        # v3plus_feature2 = self.head2(blocks)  # (b, c, h, w)
        b, c, h, w = v3plus_feature.shape
        # print("@@@@")
        # print(v3plus_feature.shape)
        # print(aspp_feat.shape)

        pred = self.classifier(v3plus_feature)
        # # aspp_feat = torch.flatten(aspp_feat, 1)
        # pred2 = self.classifier2(aspp_feat)
        # print(pred2.shape)
        # # 2,24,15,15
        # # print(pred2)
        b, c, h, w = data.shape
...
class Head(nn.Module):
    ...
    def forward(self, f_list):
        ...
        f1 = F.interpolate(f, size=(low_h, low_w), mode='bilinear', align_corners=True)
        f1 = torch.cat((f1, low_level_features), dim=1)
        f1 = self.last_conv(f1)
...
if __name__ == '__main__':
    ...
    # img_cv2_tensor=img_cv2_tensor.unsqueeze(0)
    # img_cv2_tensor=torch.stack((img_cv2_tensor, img_cv2_tensor), 0).size()
    print(img_cv2_tensor.size())
